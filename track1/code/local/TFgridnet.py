...
class GridNetV2Block(nn.Module):
    ...
    def __init__(
        ...
        # Granular time-axis causal toggles (frequency-axis intra_rnn always
        # stays bidirectional -> weights preserved):
        #   p_rnn  : inter_rnn (over time T)  bi -> uni
        #   p_attn : time self-attention      add causal mask
        # `causal` is the master default for both when the granular flags are
        # left as None.
        #
        # inter_block (BLOCK-ONLINE):
        #   When set to an int W, the time-axis inter_rnn is built FULLY
        #   BIDIRECTIONAL (identical structure to the ORIGINAL non-causal model)
        #   but at forward() time it is run independently over non-overlapping
        #   chunks of W frames. Each output frame then depends only on frames in
        #   ITS OWN chunk -> bounded look-ahead of (W-1) frames while restoring
        #   backward context. This also lets the FULL bidirectional pretrained
        #   weights (inter_rnn fwd+reverse, inter_linear 512-wide) load verbatim.
        #   inter_block takes PRECEDENCE over p_rnn for the inter_rnn structure.
        self.p_rnn = causal if p_rnn is None else p_rnn
        self.p_attn = causal if p_attn is None else p_attn
        self.causal = bool(self.p_rnn or self.p_attn)
        self.inter_block = inter_block
        if self.inter_block is not None:
            # block-online: bidirectional inter_rnn (structure == non-causal),
            # so inter_dirs=2 and the pretrained weights load verbatim.
            inter_bidirectional = True
            inter_dirs = 2
        else:
            inter_bidirectional = not self.p_rnn
            inter_dirs = 1 if self.p_rnn else 2  # forward-only LSTM when p_rnn

        self.intra_norm = nn.LayerNorm(emb_dim, eps=eps)
        # intra_rnn (frequency axis) -- ALWAYS bidirectional. Do NOT touch.
        ...
        # inter_rnn (time axis) -- uni-directional (forward) when p_rnn,
        # but FULLY bidirectional when inter_block is set (block-online).
        self.inter_rnn = nn.LSTM(
            in_channels, hidden_channels, 1, batch_first=True,
            bidirectional=inter_bidirectional,
        )
        if emb_ks == emb_hs:
            self.inter_linear = nn.Linear(hidden_channels * inter_dirs, in_channels)
        else:
            self.inter_linear = nn.ConvTranspose1d(
                hidden_channels * inter_dirs, emb_dim, emb_ks, stride=emb_hs
            )
    ...
    def forward(self, x):
        ...
        if self.emb_ks == self.emb_hs:
            ...
            if self.inter_block is not None:
                # ---- BLOCK-ONLINE bidirectional inter_rnn ----------------- #
                # Run the (bidirectional) LSTM independently over consecutive,
                # NON-OVERLAPPING chunks of W frames along the time axis. Each
                # chunk is a separate sequence in the batch dim, so the LSTM's
                # backward pass cannot reach across a chunk boundary: output
                # frame t depends ONLY on frames in [floor(t/W)*W, .. +W).
                # This bounds the future-dependency (look-ahead) to (W-1)
                # frames while restoring backward context inside the chunk.
                W = int(self.inter_block)
                BQ, Tcur, Cin = inter_rnn.shape  # [BQ, T, C]  (emb_ks==1 -> I*C==C)
                pad = (-Tcur) % W                 # right-pad T up to multiple of W
                if pad:
                    inter_rnn = F.pad(inter_rnn, (0, 0, 0, pad))  # pad on time axis
                Tpad = Tcur + pad
                nchunks = Tpad // W
                # [BQ, Tpad, C] -> [BQ*nchunks, W, C]: chunks become independent
                # sequences. .reshape (not .view) because the pad makes it
                # non-contiguous along the time split.
                inter_rnn = inter_rnn.reshape(BQ * nchunks, W, Cin)
                inter_rnn, _ = self.inter_rnn(inter_rnn)  # [BQ*nchunks, W, 2H]
                inter_rnn = self.inter_linear(inter_rnn)  # [BQ*nchunks, W, I*C]
                # back to [BQ, Tpad, C] then drop the right padding -> [BQ, T, C]
                inter_rnn = inter_rnn.reshape(BQ, Tpad, -1)
                if pad:
                    inter_rnn = inter_rnn[:, :Tcur, :].contiguous()
        ...
        attn_mat = torch.matmul(Q, K) / (emb_dim**0.5)  # [B', T, T]
        if self.p_attn:
            # query=dim1 (current frame), key=dim2 (attended frame over TIME).
            # Forbid attending to future frames (key index > query index).
            Tq = attn_mat.shape[-1]
            causal_mask = torch.tril(
                torch.ones(Tq, Tq, device=attn_mat.device, dtype=torch.bool)
            )
